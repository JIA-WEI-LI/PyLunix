...
from ..tool_button.tool_button import SubtleToolButton
...
class _BaseTextBoxEdit(QLineEdit):
    ...
    def __init__(self, text: str = "", parent=None):
        ...
        self.hBoxLayout.setContentsMargins(*inner_button_margin)
    ...
    def addAction(self, action: QAction, position: QLineEdit.ActionPosition = QLineEdit.ActionPosition.TrailingPosition):
        ...
        if position == QLineEdit.ActionPosition.LeadingPosition:
            self.hBoxLayout.insertWidget(len(self.leftButtons), button)
    ...
    def _adjustTextMargins(self):
        ...
        left_margin = sum(btn.width() + 4 for btn in self.leftButtons if btn.isVisible())
        right_margin = sum(btn.width() + 4 for btn in self.rightButtons if btn.isVisible())

        if left_margin > 0: left_margin += 4
        if right_margin > 0: right_margin += 2

        m = self.textMargins()
        self.setTextMargins(int(left_margin), m.top(), int(right_margin), m.bottom())
...
class TextBoxButton(SubtleToolButton):
    """
...
class TextBoxEdit(_BaseTextBoxEdit):
    ...
    def __init__(self, text: str = "", foreground : Optional[Union[Qt.GlobalColor, QColor, str]] = None, parent=None):
        ...
        self.rightButtons.append(self.clearButton)
    ...
    def _get_text_color(self) ->str:
        return StateColorResolver.resolve("TextControl", "Foreground", self, theme_type="TEXT_BOX")
...
class TextBox(QWidget):
    ...
    Example:
        >>> tb = TextBox(header="Name")
        >>> tb.valueChanged.connect(lambda val: print(val))
        >>> print(tb.text())
    """
    valueChanged = pyqtSignal(object)
    ...
    def setClearButtonAlwaysVisible(self, always_visible: bool=True):
        """
        Sets whether the clear button should remain visible regardless of focus.

        Args:
            always_visible (bool): If True, the clear button persists even when 
                                   the field is empty or loses focus.
        """
        self.textBoxEdit.setClearButtonAlwaysVisible(always_visible)
